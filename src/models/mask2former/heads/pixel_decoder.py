...
from src.models.mask2former.utils.batch_norm import get_norm
from src.models.mask2former.utils.position_encoding import PositionEmbeddingSine
from src.models.mask2former.utils.transformer import _get_clones, _get_activation_fn
from src.models.mask2former.utils.ms_deform_attn import MSDeformAttn
...
class MSDeformAttnPixelDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.transformer_num_feature_levels = 3  # Retained upstream implementation fixes this to three feature levels.
        if self.transformer_num_feature_levels > 1:
            input_proj_list = []
            # from low resolution to high resolution (res5 -> res2)
            for in_channels in transformer_in_channels[::-1][:-1]:  # Retained upstream behavior skips the highest-resolution feature here.
        ...
        use_bias = norm == ""
        for idx, in_channels in enumerate(self.feature_channels[:1]):  # Retained upstream behavior builds one extra FPN stage here.
            ...
            # self.add_module("lateral_convs".format(idx + 1), lateral_conv)  # Upstream note: explicit adapter_* names were not retained.
            # self.add_module("output_convs".format(idx + 1), output_conv)  # Upstream note: explicit layer_* names were not retained.
    ...
    def forward_features(self, features):
        ...
        for idx, f in enumerate(self.transformer_in_features[::-1][:-1]):  # Retained upstream behavior skips the highest-resolution feature here.
        ...
        # Reverse feature maps into top-down order (from low to high resolution)
        for idx, f in enumerate(self.in_features[0]):  # Retained upstream feature-level iteration is kept as-is here.
            x = features[f].float()
