...
class ChangeDecoder(nn.Module):
    def __init__(self, encoder_dims, channel_first, norm_layer, ssm_act_layer, mlp_act_layer, **kwargs):
        ...
        self.st_block_11 = nn.Sequential(nn.Conv2d(kernel_size=1, in_channels=encoder_dims[-4] * 2, out_channels=128),
        ...
        # Fuse layer  
        self.fuse_layer_4 = nn.Sequential(nn.Conv2d(kernel_size=1, in_channels=128 * 1, out_channels=128),
                                          nn.BatchNorm2d(128), nn.ReLU(inplace=True))
        self.fuse_layer_3 = nn.Sequential(nn.Conv2d(kernel_size=1, in_channels=128 * 1, out_channels=128),
                                          nn.BatchNorm2d(128), nn.ReLU(inplace=True))
    ...
    def forward(self, pre_features, post_features):
        ...
        p4 = self.fuse_layer_4(p41) # (torch.cat([p41, p42[:, :, :, ::2], p42[:, :, :, 1::2], p43[:, :, :, 0:W], p43[:, :, :, W:]], dim=1))
        ...
        p3 = self.fuse_layer_3(p31) # torch.cat([p31, p32[:, :, :, ::2], p32[:, :, :, 1::2], p33[:, :, :, 0:W], p33[:, :, :, W:]], dim=1))
        ...
        p2 = self.fuse_layer_2(p21) # torch.cat([p21, p22[:, :, :, ::2], p22[:, :, :, 1::2], p23[:, :, :, 0:W], p23[:, :, :, W:]], dim=1))
        ...
        p1 = self.fuse_layer_1(p11) # torch.cat([p11, p12[:, :, :, ::2], p12[:, :, :, 1::2], p13[:, :, :, 0:W], p13[:, :, :, W:]], dim=1))
