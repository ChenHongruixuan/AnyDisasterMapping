...
def get_reference_points(spatial_shapes, device):
    ...
    ref = torch.stack((ref_x, ref_y), -1) # Normalized coordinates for each point, shape (1, H_*W_, 2)
    #reference_points_list.append(ref)
    #reference_points = torch.cat(reference_points_list, 1) # Concatenate coordinates across feature maps
...
def deform_inputs_func(x, num_tokens):
    ...
    spatial_shapes = torch.as_tensor([int(math.sqrt(num_tokens)), int(math.sqrt(num_tokens))],
                                    dtype=torch.long, device=x.device) # 3x2 tensor
    # level_start_index = torch.cat((spatial_shapes.new_zeros(
    #     (1,)), spatial_shapes.prod(1).cumsum(0)[:-1])) # Starts with 0, then cumulative flattened sizes
    ...
    # deform_inputs stores spatial shapes and one reference-point grid.
    # The injector query is single-scale while the value path is multi-scale.
...
class SampleAttention(nn.Module):
    def __init__(
        ...
        self.sampling_offsets = nn.Linear(all_head_dim, self.num_heads  * n_points * 2) # Sample n_points offsets per head
    ...
    def forward(self, x, H, W, deform_inputs):
        ...
        offset_normalizer =  torch.stack([input_spatial_shapes[1], input_spatial_shapes[0]]) # Flip coordinate order to xy

        sampling_locations = reference_points[:, None, :, None, :] \
                                + sampling_offsets / offset_normalizer[None, None, None, None, :] # Normalize xy offsets per level
        ...
        sampled_k = F.grid_sample(k, sampling_locations, mode='bilinear',
                                          padding_mode='zeros', align_corners=False).reshape(B, self.num_heads, L // self.num_heads, N, self.n_points).permute(0,1,3,4,2) # Sample keys from each level at the queried locations
        
        # B*H, c, N, P
        sampled_v = F.grid_sample(v, sampling_locations, mode='bilinear',
                                          padding_mode='zeros', align_corners=False).reshape(B, self.num_heads, L // self.num_heads, N, self.n_points).permute(0,1,3,4,2) # Sample values from each level at the queried locations
...
class SpectralVisionTransformer(nn.Module):
    ...
    def __init__(self, NUM_TOKENS=None, img_size=224, in_chans=3, embed_dim=768, depth=12,
                 ...
                 out_indices=[11], interval=3, pretrained=None, restart_regression=True, n_points=8):
        super().__init__()
        ...
        #     self.conv_q.append(nn.Conv2d(NUM_TOKENS, NUM_TOKENS, kernel_size=1))
        #     self.conv_k.append(nn.Conv1d(NUM_TOKENS, NUM_TOKENS, kernel_size=1))
        #     self.conv_v.append(nn.Conv1d(NUM_TOKENS, NUM_TOKENS, kernel_size=1))
    ...
    def forward_features(self, x):
        ...
        deform_inputs = deform_inputs_func(x, self.NUM_TOKENS)
        ...
        for i, blk in enumerate(self.blocks):
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, H, W, deform_inputs)
            else:
                x = blk(x, H, W, deform_inputs)
...
class altered_SpecViT_0428(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage"""
    ...
    def __init__(self, NUM_TOKENS=None, img_size=224, in_chans=3, embed_dim=768, depth=12,
        ...
        self.l1 = nn.Linear(embed_dim, 128, bias=False)
        self.l2 = nn.Linear(embed_dim, 128, bias=False)
        self.l3 = nn.Linear(embed_dim, 128, bias=False)
        self.l4 = nn.Linear(embed_dim, 128, bias=False)
    ...
    def forward_features(self, x):
        ...
        ops = [self.l1,self.l2,self.l3,self.l4]
        for i in range(len(ops)):
            features[i] = ops[i](features[i])
...
def spec_vit_b_rvsa(args, inchannels=3):
...
def spec_vit_l_rvsa(args, inchannels=3):
...
def spec_vit_h_rvsa(args, inchannels=3):
