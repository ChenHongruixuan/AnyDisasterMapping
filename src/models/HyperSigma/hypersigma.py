...
from src.models.HyperSigma.SpatViT import ALter_SpatViT

import torch.nn.functional as F
import torch
from src.models.HyperSigma.SpecViT_new import altered_SpecViT_0428

class HyperSigma(nn.Module):
    def __init__(self, in_channels, num_classes=2, use_checkpoint=True,
                 img_size=128,
                 post_channels=None,
                 spatial_pretrained='pretrained_weight/HSI_spatial_checkpoint-1600.pth',
                 spectral_pretrained='pretrained_weight/spec-vit-base-ultra-checkpoint-1599.pth'):
        super(HyperSigma, self).__init__()
        post_ch = post_channels if post_channels is not None else in_channels
        if in_channels == post_ch:
            # Symmetric: shared dim_red (backward compatible)
            self.dim_red = nn.Conv2d(kernel_size=1, in_channels=in_channels, out_channels=100)
        else:
            # Asymmetric: separate dim_red for pre and post branches
            self.dim_red = None
            self.dim_red_pre = nn.Conv2d(kernel_size=1, in_channels=in_channels, out_channels=100)
            self.dim_red_post = nn.Conv2d(kernel_size=1, in_channels=post_ch, out_channels=100)
        self.spat_encoder  = ALter_SpatViT(img_size=img_size,
                                            in_chans=100,
                                            ...
                                            interval=3, pretrained=spatial_pretrained)
        NUM_TOKENS = 100
        self.spec_encoder = altered_SpecViT_0428(
            ...
            use_checkpoint=use_checkpoint,
            use_abs_pos_emb=False,
            interval=3, pretrained=spectral_pretrained
        )
    ...
    def forward(self, x, y):
        ...
        if self.dim_red is not None:
            ...
            x = self.dim_red_pre(x)
            y = self.dim_red_post(y)
