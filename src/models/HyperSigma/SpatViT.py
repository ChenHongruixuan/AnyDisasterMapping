...
def deform_inputs_func(x):
    B, c, h, w = x.shape
    b = B // 3
    # Input img_size = 16; patch_size = 8.
    spatial_shapes = torch.as_tensor([h // 8, w // 8],
                                     dtype=torch.long, device=x.device)  # added ,2024.1.7
    reference_points = get_reference_points([h // 8, w // 8], x.device)

    # Input img_size = 32; patch_size = 16.
...
class SpatViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
        ...
        if patch_size == 16:
            ...
            self.fpn4 = nn.Sequential(
                nn.MaxPool2d(kernel_size=4, stride=4),  # defalut
            )
    ...
    def forward_features(self, x):
        ...
        deform_inputs = deform_inputs_func(x)
...
# Alteration notes:
# 1) In SpatViT.init_weights(self, pretrained), num_extra_tokens is changed from 1 to 0.
# 2) For patch_size == 8, fpn1 is changed to max pooling with kernel=2 and stride=2.
class ALter_SpatViT(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        if patch_size == 16:
            ...
            self.fpn4 = nn.Sequential(
                nn.MaxPool2d(kernel_size=2, stride=2),  # For img_size = 16 and patch_size = 8, deform_inputs_func divides by 8.
                # self.fpn4 = nn.Sequential(
                #     nn.MaxPool2d(kernel_size=4, stride=4),  # defalut
            )
...
if __name__ == "__main__":
    ...
    input = torch.Tensor(2, in_chans, img_size, img_size).cuda()
