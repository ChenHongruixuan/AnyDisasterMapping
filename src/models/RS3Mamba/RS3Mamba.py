...
class RS3Mamba(nn.Module):
    def __init__(self,
        ...
        encoder_cfg: Dict[str, object] = dict(encoder_kwargs) if encoder_kwargs is not None else {}
        if 'in_chans' not in encoder_cfg or encoder_cfg['in_chans'] is None:
            encoder_cfg['in_chans'] = in_channels
        out_indices = encoder_cfg.pop('out_indices', (0, 1, 2, 3))
        self.vssm_encoder = Backbone_VSSM(
            out_indices=out_indices,
            pretrained=encoder_pretrained,
            **encoder_cfg,
        )
        encoder_channels = self.backbone.feature_info.channels()
        if not hasattr(self.vssm_encoder, 'dims'):
            raise AttributeError('Backbone_VSSM is expected to expose `dims`; please verify encoder configuration.')
        ssm_dims = list(self.vssm_encoder.dims)

        self.Fuse = nn.ModuleList()
        self.decoder = Decoder(encoder_channels, decode_channels, dropout, window_size, num_classes)
        for i in range(4):
            fuse = FusionBlock(encoder_channels[i], ssm_dims[i], window_size=window_size)
            self.Fuse.append(fuse)
        

    def forward(self, x):
        h, w = x.size()[-2:]
        vss_feats = self.vssm_encoder(x)
        ress = []
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.act1(x)
