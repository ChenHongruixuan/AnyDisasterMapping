...
try:  # pragma: no cover - optional API depending on torchvision version
    from torchvision.models import get_model_weights
except ImportError:  # pragma: no cover - torchvision<0.13 fallback
...
class SiamCRNN(nn.Module):
    def __init__(self, num_classes, backbone: str = 'resnet18', pretrained: bool = True, siamese_mode: str = 'pseudo', in_channels: int = 3, post_channels: int = None):
        super(SiamCRNN, self).__init__()

        self.backbone = backbone
        self.siamese_mode = self._normalize_mode(siamese_mode)
        self.shared_encoder = self.siamese_mode == 'siamese'

        post_ch = post_channels if post_channels is not None else in_channels

        encoder_pre = self._build_encoder(backbone, pretrained)
        expansion = self._infer_expansion(encoder_pre)
        stage_channels = [64 * expansion, 128 * expansion, 256 * expansion, 512 * expansion]
        c1, c2, c3, c4 = stage_channels

        if in_channels != 3:
            from src.models.channel_adapt import adapt_conv_channels
            encoder_pre.conv1 = adapt_conv_channels(encoder_pre.conv1, in_channels)
        ...
        self.extractor_pre = create_feature_extractor(encoder_pre, return_nodes=return_nodes)
        if self.shared_encoder:
            self.extractor_post = self.extractor_pre
        else:
            encoder_post = self._build_encoder(backbone, pretrained)
            if post_ch != 3:
                from src.models.channel_adapt import adapt_conv_channels
                encoder_post.conv1 = adapt_conv_channels(encoder_post.conv1, post_ch)
            self.extractor_post = create_feature_extractor(encoder_post, return_nodes=return_nodes)

        self.convlstm_4 = ConvLSTM(input_dim=c4, hidden_dim=128, kernel_size=(3, 3), num_layers=1,
                                   batch_first=True)
        self.convlstm_3 = ConvLSTM(input_dim=c3, hidden_dim=128, kernel_size=(3, 3), num_layers=1,
                                   batch_first=True)
        self.convlstm_2 = ConvLSTM(input_dim=c2, hidden_dim=128, kernel_size=(3, 3), num_layers=1,
                                   batch_first=True)
        self.convlstm_1 = ConvLSTM(input_dim=c1, hidden_dim=128, kernel_size=(3, 3), num_layers=1,
                                   batch_first=True)

        self.trans_layer_4 = nn.Sequential(nn.Conv2d(kernel_size=3, in_channels=c4, out_channels=128, padding=1),
                                           nn.BatchNorm2d(128), nn.ReLU())
        self.trans_layer_3 = nn.Sequential(nn.Conv2d(kernel_size=3, in_channels=c3, out_channels=128, padding=1),
                                           nn.BatchNorm2d(128), nn.ReLU())
        self.trans_layer_2 = nn.Sequential(nn.Conv2d(kernel_size=3, in_channels=c2, out_channels=128, padding=1),
                                           nn.BatchNorm2d(128), nn.ReLU())
        self.trans_layer_1 = nn.Sequential(nn.Conv2d(kernel_size=3, in_channels=c1, out_channels=128, padding=1),
                                           nn.BatchNorm2d(128), nn.ReLU())
    ...
    @staticmethod
    def _infer_expansion(encoder: nn.Module) -> int:
        # ResNet basic blocks expose `expansion` indicating the widening factor per stage.
        block = getattr(encoder, 'layer1')[0]
        return getattr(block, 'expansion', 1)

    @staticmethod
    def _normalize_mode(mode: str) -> str:
        normalized = mode.lower()
        if normalized not in {'siamese', 'pseudo'}:
            raise ValueError("siamese_mode must be either 'siamese' or 'pseudo'")
        return normalized

    def _build_encoder(self, backbone: str, pretrained: bool) -> nn.Module:
        ...
        if get_model_weights is not None:
            try:
                ...
                kwargs['weights'] = weights_enum.DEFAULT if pretrained else None
        ...
        try:
            ...
            encoder = constructor(pretrained=pretrained)
        return encoder
    ...
    def forward(self, pre_data, post_data):
        pre_features = self.extractor_pre(pre_data)
        post_features = self.extractor_post(post_data)
