...
DEST_ROOT="${1:-data/flood/kurosiwo/raw/data}"
...
for i in "${!urls[@]}"; do
    ...
    if wget -c "$file_url" -O "$DEST_ROOT/$filename"; then
      echo "$file_url downloaded successfully"
      downloaded_files+=("$DEST_ROOT/$filename")
    else
...
for i in "${!downloaded_files[@]}"; do
    ...
    if tar -xvf "$file" --directory="$DEST_ROOT"; then
...
if [ ${#failed_extractions[@]} -ne 0 ]; then
    ...
    for i in "${!failed_extractions[@]}";do
        file=${failed_extractions[i]}
        echo "$file"
    done
    flag=$((flag + 1))
fi
...
if [ ${flag} -eq 0 ]; then
    echo "KuroSiwo downloading finished successfully into: $DEST_ROOT"
fi
